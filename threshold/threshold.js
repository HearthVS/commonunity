...
(function () {
  ...
  async function applyBrandManifest() {
    try {
      const res = await fetch('/api/brand/manifest', { credentials: 'same-origin', cache: 'no-store' });
      if (!res.ok) return;
      const manifest = await res.json();
      const p = manifest.logo_palette || {};
      const rootStyle = document.documentElement.style;
      const map = {
        center: '--brand-logo-center',
        north: '--brand-logo-north',
        east: '--brand-logo-east',
        south: '--brand-logo-south',
        west: '--brand-logo-west',
        inner_north: '--brand-logo-inner-north',
        inner_east: '--brand-logo-inner-east',
        inner_south: '--brand-logo-inner-south',
        inner_west: '--brand-logo-inner-west'
      };
      Object.keys(map).forEach((key) => {
        if (p[key]) rootStyle.setProperty(map[key], p[key]);
      });
      if (p.north) rootStyle.setProperty('--om-gold', p.north);
    } catch (_) {}
  }
