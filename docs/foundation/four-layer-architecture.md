...
This is the positive frame that the trust messaging rewrite calibrates against. (The dedicated trust-messaging audit doc is not yet on `main`; when it lands under `docs/handoffs/audits/`, link it here.)
...
- **By the trust messaging rewrite and the other revision areas**: the sanctuary clause and the consume → live → serve arc are the positive frames the rewrites calibrate against, alongside the Charter.
